...
def handlingShorturl(request, **kwargs):
    ...
    query="select long_link from users where short_link='" + url + "'"
...
def login(request):
    ...
    if data is None:
        ...
        if data[1] == psw:
            data = {"email": "Logged in successfully", "password": ""}
            cursor = connection.cursor()
            ...
            data = {"email": "incorrect password", "password": ""}
            return render(request, "first.html", data)
...
def urlshortner(request):
    ...
    if customUrl is None or customUrl==" ":
        ...
        cursor = connection.cursor()
        query1 = "select * from links where short_links ='" + customUrl + "'"
        cursor.execute(query1)
        data = cursor.fetchone()
        if data is not None:
            data= {"email": "Custom URL Already exists please try some other url"}
            return render(request, "first.html", data)
        else:
